...
open(10,file='input_diffusion.dat',action='READ')
...
call norm_calculator ( L1, L2 ,L_infinity &
                                           ,ntime ,ncell, error, conc_exact,conc_result,dx,dt,theta,i_case)
...
do ivar=1,ncell+1
   ...
   ks_lo(ivar,jvar+1) = one
   ks_hi(ivar,jvar+1) = one
...
do ivar=1,ncell
    do jvar=1,ntime
        conc_exact(ivar,jvar)=two*((ivar-1)*dx+1.0d-1)+two*two*cos(half*pi*((ivar-1.0d0)*dx+1.0d-1))*exp(-(jvar)*dt*(pi**2.0d0)/4.0d0)
    end do
end do
...
if (i_case == 3)then
    ...
    do ivar=1,ncell
        do jvar=10,ntime  !!!!!!!!!!!!!!!! 10 instead of one 
        
            conc_exact(ivar,jvar)= exp(minus * ((((1-ncell)/2+ivar-1)*dx)**two) / 4.0/ jvar/dt/1)/sqrt (4.0d0*1*pi*jvar*dt) ! 1 instead of KS
    ...
    do ivar2=1,ncell
        do jvar2=10,ntime   !!! 10 instead of one
            error(ivar2,jvar2)= abs(conc_exact(ivar2,jvar2)- conc_result(ivar2,jvar2))
        end do
    end do
end if 
...
open (100,file= 'errorcomp.dat',status= 'unknown')
write (100,*)  'first column is place (X) and the second is error'
do kvar=10,ntime  !!!!!!!! 10 instead of one
    write (100,*) kvar,'time'
    write (100,*) kvar*dt
    do ivar2=1,ncell
    write (100,*)  (((1-ncell)/2+ivar2-1)*dx), error (ivar2,kvar)
   end do
