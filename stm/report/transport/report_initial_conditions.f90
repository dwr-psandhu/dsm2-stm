...
module report_initial_conditions
    ...
    !> Fill array with 1D rectanglar shape
    !> This routine expects a 1D array, so multi-constituents
    !> have to be initialized separately
    subroutine fill_rectangular(vals,nloc,nlo,nhi,fill,fill_else)
        use stm_precision
        implicit none
        real(STM_REAL), intent(out) :: vals(nloc)  !< values to be filled
        integer,        intent(in)  :: nloc        !< size of array
        integer,        intent(in)  :: nlo         !< lo side index
        integer,        intent(in)  :: nhi         !< hi side index
        real(STM_REAL), intent(in)  :: fill        !< fill with this value between nlo and nhi
        real(STM_REAL), intent(in)  :: fill_else   !< fill with this value if not between nlo and nhi
