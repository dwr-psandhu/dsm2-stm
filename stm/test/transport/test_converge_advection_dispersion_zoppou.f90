...
real(stm_real),parameter :: x_left = 10000.0d0        !< Left hand side of the channel zoppou tese
real(stm_real),parameter :: x_right = 15000.0d0       !< Right hand side of the channel zoppou test
real(stm_real),parameter :: x_l_time = zero           !< Left hand side of the channel time dependent test
real(stm_real),parameter :: x_r_time = nine*two       !< Right hand side of the channel time dependent test
real(stm_real),parameter :: start_time = 3000.0d0     !< Starts at 100000 sec (second) zoppou test
real(stm_real),parameter :: end_time = 5000.0d0       !< Ends at 190000 (second)zoppou test
real(stm_real),parameter :: start_t_time = 0.1d0      !< Starts at 0.1 sec (second)time dependent test
real(stm_real),parameter :: end_t_time = three        !< Ends at 3 (second)time dependent test
real(stm_real),parameter :: a0 = 1.0d7                !< Constant of area A=A0*(x^-1)
real(stm_real),parameter :: a0_t = 1.6d0              !< Constant of area time dependent flow
real(stm_real),parameter :: c0 = one                  !< Constant concentration
...
real(stm_real),parameter :: d0_t = 1.5d0              !< Constant of dispersion coefficent for time dependent test
real(stm_real),parameter :: u0_t = 1.2d0              !< Constant of velocity for time dependent test
real(stm_real),parameter :: omega = 12.41d0           !< Omega in velocity and diffusivity of time dependent test
...
real(stm_real),intent(out):: value_time_dep !< Dirichlet initial condition at left side of channel
real(stm_real),intent(in) :: xpos           !< Location where data is requested
real(stm_real),intent(in) :: time           !< Time
...
integer,intent(in) :: nconc 
integer,intent(in) :: nx_base 
real(stm_real),intent(out):: bc_value_time(nconc)       !< Dirichlet initial condition at left side of channel
real(stm_real),intent(in) :: xloc                       !< Location where data is requested
real(stm_real),intent(in) :: time                       !< Time
real(stm_real),intent(in) :: dt                         !< Time step
real(stm_real),intent(in) :: dx                         !< Spacial mesh size
